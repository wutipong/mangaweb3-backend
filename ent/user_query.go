...
func (uq *UserQuery) QueryFavoriteItems() *MetaQuery {
	query := (&MetaClient{config: uq.config}).Query()
...
// WithFavoriteTags tells the query-builder to eager-load the nodes that are connected to
// the "favorite_tags" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFavoriteItems; query != nil {
		if err := uq.loadFavoriteItems(ctx, query, nodes,
			func(n *User) { n.Edges.FavoriteItems = []*Meta{} },
			func(n *User, e *Meta) { n.Edges.FavoriteItems = append(n.Edges.FavoriteItems, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withFavoriteTags; query != nil {
...
func (uq *UserQuery) loadFavoriteItems(ctx context.Context, query *MetaQuery, nodes []*User, init func(*User), assign func(*User, *Meta)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(meta.FieldID), joinT.C(user.FavoriteItemsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Meta](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
...
func (uq *UserQuery) loadFavoriteTags(ctx context.Context, query *TagQuery, nodes []*User, init func(*User), assign func(*User, *Tag)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.FavoriteTagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(user.FavoriteTagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.FavoriteTagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.FavoriteTagsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "favorite_tags" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadProgress(ctx context.Context, query *ProgressQuery, nodes []*User, init func(*User), assign func(*User, *Progress)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(progress.FieldUserID)
	}
