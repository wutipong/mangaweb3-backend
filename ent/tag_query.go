...
func (tq *TagQuery) loadMeta(ctx context.Context, query *MetaQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Meta)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.MetaTable)
		s.Join(joinT).On(s.C(meta.FieldID), joinT.C(tag.MetaPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.MetaPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.MetaPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "meta" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
